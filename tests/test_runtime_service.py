...
class RuntimeMediaServiceTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            svc.repo.set_workspace_state(
                TRANSLATION_CONFIG_STATE_KEY,
                {
                    ...
                    "providers": [
                        {"id": "m2m100", "enabled": True, "credentials": {}},
                ...
                with patch.object(
                    ...
                    return_value={
                        ...
                        "sentence_node": {
                            ...
                            "node_id": "n1",
                            "translations": {
                                "m2m100": {"source_lang": "en", "target_lang": "ru", "text": "Она доверяла ему."}
                            },
                            "linguistic_elements": [],
                        },
                    },
    ...
    def test_visualizer_payload_falls_back_to_backend_translation_when_selected_missing(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            sentence_node = payload["She trusted him."]
            self.assertEqual(sentence_node["translations"]["m2m100"]["text"], "Она доверяла ему.")
    ...
    @patch("ela_pipeline.inference.run.run_pipeline")
    def test_build_sentence_contract_enriches_sentence_notes_with_subject_predicate_focus(self, mock_run_pipeline):
    ...
    @patch("ela_pipeline.inference.run.run_pipeline")
    @patch(
        "ela_pipeline.runtime.service.os.path.isfile",
    ...
    @patch("ela_pipeline.runtime.service.os.path.isdir", return_value=True)
    def test_build_sentence_contract_auto_falls_back_to_rule_when_metadata_missing(
        self,
        _mock_isdir,
        _mock_isfile,
        mock_run_pipeline,
                    ...
                    contract_sentences=[
                        {
                            ...
                            "sentence_node": {
                                ...
                                "node_id": "n-new",
                                "translations": {
                                    "m2m100": {"source_lang": "en", "target_lang": "ru", "text": "Она снова доверяла ему."}
                                },
                                "linguistic_elements": [],
                            },
                        }
