...
CONTRACT_BUILD_VERSION = "2026-04-11-contraction-fix-v1"
...
class LocalJobCanceled(Exception):
    """Raised to stop backend local job execution after cancellation."""
...
class RuntimeMediaService:
        ...
        *,
        ...
        if raw.get("route") == "local":
            if async_local_processing:
                raw["job_id"] = self._start_local_processing_job(
                    media_path=media_path,
                    project_id=effective_project_id,
                    media_file_id=media_file_id,
                    translation_provider=selected_provider,
    ...
    def delete_analysis(self, *, document_id: str) -> dict[str, Any]:
        ...
        upload_base = (Path(os.getenv("MEDIA_TEMP_DIR", "artifacts/media_tmp")) / "uploads").resolve()
        if source_path:
            try:
                source_candidate = Path(source_path).resolve()
                if str(source_candidate).startswith(str(upload_base)) and source_candidate.exists() and source_candidate.is_file():
                    source_candidate.unlink(missing_ok=True)
                    removed_source_file = True
            except Exception:
                removed_source_file = False

        removed_media_file = False
        if media_file_id:
            remaining_rows = [row for row in self.repo.list_analysis_history() if str(row.get("media_file_id") or "").strip() == media_file_id]
            if not remaining_rows:
                removed_media_file = bool(self.repo.delete_media_file(media_file_id))
        ...
        return {
            ...
            "removed_source_file": bool(removed_source_file),
            "removed_media_file": bool(removed_media_file),
        }
    ...
    def _stage_progress(stage_name: str, ratio: float | None = None) -> list[int]:
        ...
        if ratio is not None:
            ...
            if stage_name in {"translating_text", "linguistic_parsing"}:
        ...
        mapping = {
            ...
            "linguistic_parsing": [100, 100, 65, 0, 0],
    ...
    def _start_local_processing_job(
        ...
        *,
        ...
        self._set_local_job_state(
            ...
            message="Backend media processing started.",
            stage_name="queued",
        )
            ...
            try:
                ...
                if result.get("status") == "completed":
                    ...
                    self._set_local_job_state(
                        job_id=job_id,
                        status="rejected",
                        message=str(result.get("message") or "Backend media processing failed."),
                        stage_name="error",
                        document_id=document_id,
                    )
