"""
===========================================================
T5 Training Script (CPU) — FULL DATA + ROUGE (FIXED & STABLE)
...
FULL_SAVE_STEPS = 100  # 500  # < eval_steps, чтобы save был раньше
FULL_EVAL_STEPS = 100  # 500
FULL_LOGGING_STEPS = 100
SAVE_TOTAL_LIMIT = 3

MODEL_TAG = MODEL_NAME.replace("/", "_")
RESULTS_DIR = os.path.join(BASE_DIR, f"results_llm_notes_v3_{MODEL_TAG}_cpu")
...
def compute_metrics(eval_pred):
    ...
    try:
        ...
        # Лучшая практика для ROUGE: join words with space, no extra newlines
...
def main() -> None:
    try:
        ...
        args = Seq2SeqTrainingArguments(
            ...
            save_strategy="steps",
            save_steps=500,  # ← было 400, теперь 500
        ...
        logging.info(f"ARGS CHECK: output_dir={args.output_dir}")
        logging.info(
            f"ARGS CHECK: eval_strategy={args.eval_strategy}, eval_steps={args.eval_steps}"
        )
        logging.info(
            f"ARGS CHECK: save_strategy={args.save_strategy}, save_steps={args.save_steps}"
        )
