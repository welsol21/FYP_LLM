...
- sentence-level `linguistic_notes` are generated by deterministic runtime logic first; external ChatGPT is optional (opt-in only)
- `Qwen2.5-7B-Instruct` is marked as a promising local candidate for future notes generation experiments (not production default)
...
### Sentence Notes Runtime Policy
- Default: deterministic sentence-notes builder in backend code (`ela_pipeline/runtime/sentence_notes.py`).
- ChatGPT path: disabled by default; enabled only when both conditions are true:
  - `ELA_SENTENCE_NOTES_USE_CHATGPT=1`
  - `OPENAI_API_KEY` is set
- If ChatGPT output fails quality checks, runtime falls back to deterministic notes.

Recommended defaults in `.env`:
```bash
ELA_SENTENCE_NOTES_USE_CHATGPT=0
ELA_SENTENCE_NOTES_OPENAI_MODEL=gpt-4o-mini
```
