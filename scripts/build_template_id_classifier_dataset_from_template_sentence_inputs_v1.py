...
DEFAULT_SOURCE_JSONL = "data/processed_sentence_seed/seed_preserving_sentence_dataset_v45_paired_template_sentence_nodes_contractfix4_v1/all.jsonl"
DEFAULT_OUTPUT_DIR = "data/processed_sentence_seed/seed_preserving_sentence_dataset_v48_template_id_from_template_input_v1"
...
def _label_and_reason(input_text: str, note_text: str) -> tuple[str, str]:
    """
    Heuristic labeler for template-first sentence inputs.
    Output label is a single template_id.
    """
...
def build_dataset(source_jsonl: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    report = {
        "builder": "build_template_id_classifier_dataset_from_template_sentence_inputs_v1.py",
        ...
        "unique_target": len({row["template_id"] for row in out}),
        "input_template_rows": len(out),
        "raw_input_rows": 0,
        "target_field": "template_id",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a template-id classifier dataset from template sentence inputs.")
