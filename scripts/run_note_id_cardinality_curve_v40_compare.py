...
def _build_rows(dataset_dir: Path, *, note_mode: str) -> list[dict[str, Any]]:
    rows = _load_jsonl(dataset_dir / "all.jsonl")
    ...
    for row in rows:
        ...
        if note_mode == "raw" and is_template:
            continue
        if note_mode == "template" and not is_template:
            continue
        out.append(
            {
                "input": input_text,
                ...
                "note_text": note_text,
                "note_type": note_mode,
                "dataset_name": f"v40_{note_mode}",
                "grammar_evidence": {},
                "grammar_classes": [],
                "provenance": {
                    "dataset_source": f"note_id_curve_v40_{note_mode}",
                    "treebank": f"v40_{note_mode}",
                },
            }
        )
...
def _build_subset(
    *,
    ...
    ordered = [note_id for note_id, _ in sorted(support.items(), key=lambda item: (-item[1], item[0]))]
    allowed = set(ordered[:note_count])
...
def _plot_compare(report: dict[str, Any], *, output_path: Path) -> None:
    ...
    for mode, color in (("raw", "#1b6ca8"), ("template", "#c1582a")):
        ...
        y = [float(row["test_metrics"]["top1_accuracy"]) for row in results]
        plt.plot(x, y, marker="o", label=f"{mode} top-1", color=color)
    plt.xlabel("Unique note_ids")
    plt.ylabel("Test top-1 accuracy")
    plt.title("Raw vs Template Note-ID Curve (v40)")
...
def _run_mode(
    *,
    ...
    all_rows = _build_rows(dataset_dir, note_mode=mode)
    support = Counter(str(row.get("note_id") or "") for row in all_rows)
    eligible_ids = [note_id for note_id, count in sorted(support.items(), key=lambda item: (-item[1], item[0])) if count >= min_support]
    max_count = len(eligible_ids)
    checkpoints: list[int] = []
    current = start_note_count
    while current < max_count:
        checkpoints.append(current)
        current += step
    if max_count >= start_note_count and max_count not in checkpoints:
        checkpoints.append(max_count)

    results: list[dict[str, Any]] = []
    for note_count in checkpoints:
        subset_dir = working_dir / mode / f"subset_{note_count:03d}"
        split_rows, subset_stats = _build_subset(
            all_rows=[row for row in all_rows if str(row.get("note_id") or "") in set(eligible_ids[:note_count])],
    ...
    return {
        ...
        "mode": mode,
        "input_type": "paired_template_sentence_input",
        "eligible_note_ids": max_count,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare raw vs templated note_id cardinality curves on v40 datasets.")
    parser.add_argument(
        "--raw-dataset-dir",
    ...
    parser.add_argument("--working-dir", default="artifacts/note_id_cardinality_curve_v40_compare_v1")
    ...
    template_report = _run_mode(
        dataset_dir=Path(args.template_dataset_dir),
        mode="template",
        start_note_count=args.start_note_count,
        step=args.step,
        min_support=args.min_support,
    ...
    report = {
        "experiment": "note_id_cardinality_curve_v40_compare_v1",
        "feature_profile": args.feature_profile,
        "start_note_count": args.start_note_count,
        "step": args.step,
        "min_support": args.min_support,
    ...
    _plot_compare(report, output_path=working_dir / "compare_top1.png")
